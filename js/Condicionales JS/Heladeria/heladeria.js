// const gusto1 = prompt("Ingrese el primer gusto de helado");
// const gusto2 = prompt("Ingrese el segundo gusto de helado");
// const gusto3 = prompt("Ingrese el tercer gusto de helado");

// alert(`Aquí tiene su helado de ${gusto1}, ${gusto2} y ${gusto3}`)

//Crear un programa que pida al usuario ingresar, por separado, tres gustos de helado, y luego muestre un mensaje que diga, por ejemplo, "Aquí tiene su helado de chocolate, dulce de leche y menta granizada".

const gusto1 = prompt("Elija un gusto de helado");
const gusto2 = prompt("Elija otro gusto de helado");
const gusto3 = prompt("Elija el último gusto de helado");

alert(`Su helado es de: ${gusto1}, ${gusto2} y ${gusto3}`);
